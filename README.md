#Source for Datasets:
The yelp dataset ("yelp_business.json"):
https://www.yelp.com/dataset
The Food Violation record dataset (renamed as "boston_raw.csv" before uploading to hdfs):
...
Found 2 items
drwx------+  - zs1113 zs1113          0 2021-04-21 19:21 .Trash
drwxrwx---+  - zs1113 zs1113          0 2021-04-21 19:24 hiveInput
```
...
SELECT COUNT(DISTINCT business_id) AS n, COUNT(name) AS total,  COUNT(DISTINCT business_id)/COUNT(name) uniqueness_rate FROM temp;

+-------+--------+---------------------+
|   n   | total  |   uniqueness_rate   |
+-------+--------+---------------------+
| 2031  | 2303   | 0.8818931828050369  |
+-------+--------+---------------------+
```

```
DROP TABLE unique_temp;

CREATE TABLE unique_temp AS SELECT business_id, name FROM temp GROUP BY business_id, name HAVING (COUNT(name) < 2);

SELECT COUNT(DISTINCT business_id) AS n, COUNT(name) AS total,  COUNT(DISTINCT business_id)/COUNT(name) uniqueness_rate FROM unique_temp;
...
```
DROP TABLE boston_stats;

CREATE TABLE boston_stats AS SELECT unique_temp.business_id, unique_temp.name, temp.address, temp.city, temp.stars, temp.review_count, temp.is_open, temp.n_pass, temp.n_fail, temp.pass_rate, temp.latitude, temp.longitude FROM unique_temp LEFT JOIN temp ON unique_temp.business_id=temp.business_id AND unique_temp.name = temp.name;

SELECT COUNT(DISTINCT business_id) AS n, COUNT(name) AS total,  COUNT(DISTINCT business_id)/COUNT(name) uniqueness_rate FROM boston_stats;
+-------+--------+------------------+
|   n   | total  | uniqueness_rate  |
+-------+--------+------------------+
| 1815  | 1815   | 1.0              |
